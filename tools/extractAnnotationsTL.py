...
TLClass = ['go', 'goLeft', 'stop', 'stopLeft', 'warning', 'warningLeft', 'goForward']
...
savePath = os.path.join(basePath, '_cropped')
...
for line in csv:
	...
	widthNew = width * 64 / 60
	heightNew = height * 64 / 60
	side = max(widthNew,heightNew)
	#print(side)
	#print(width)
	#print(height)    
	marginWidth = ((side - width) / width / 2) * 100
	marginHeight = ((side - height) / height / 2) * 100
	#print(marginWidth)
	#print(marginHeight)
	box = [int(fields[2])-width*marginWidth/100, int(fields[3])-height*marginHeight/100, int(fields[4])+width*marginWidth/100, int(fields[5])+height*marginHeight/100]
	...
	if args.mode == 'crop':
		filename = os.path.join(savePath, '%s.%d.%s' % (fields[1], counter[fields[1]], 'jpg'))
	else:
...
print("Done. Processed all annotations.")
